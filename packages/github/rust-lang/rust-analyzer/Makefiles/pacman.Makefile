...
CC = cargo
CFLAGS = 
DEPENDENCIES = python3 git base-devel make gcc curl cargo rustc npm nodejs
...
install-rustup:
	## Install package using rustup
	@rustup component add ${PKG_NAME}

install-all: clone
    ## Install the LSP server and the VS Code plugin
	@cd ${PKG_NAME}; ${CC} ${CFLAGS} xtask install && \
